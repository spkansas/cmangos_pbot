...
void PlayerbotMgr::OnBotLogin(Player * const bot)
{
    ...
    *pCMSG_SET_ACTIVE_MOVER << bot->GetObjectGuid();
    bot->GetSession()->QueuePacket(std::move(std::make_unique<WorldPacket>(*pCMSG_SET_ACTIVE_MOVER)));
    ...
    *pMSG_MOVE_FALL_LAND << bot->GetMover()->m_movementInfo;
    bot->GetSession()->QueuePacket(std::move(std::make_unique<WorldPacket>(*pMSG_MOVE_FALL_LAND)));
...
void Creature::LoadBotMenu(Player *pPlayer)
{
    ...
    QueryResult *result = CharacterDatabase.PQuery("SELECT DISTINCT c.guid, c.name, c.online, c.race, c.class, c.map FROM %s WHERE (c.account='%d' %s'%u') LIMIT 20", fromTable.c_str(), accountId, wherestr.c_str(), guid);
    if (!result)
    {
        delete result;
        return;
    ...
    do
    {
        ...
        else
        {
            ...
            if (pPlayer->GetPlayerbotMgr()->GetPlayerBot(guidlo) == nullptr && online == 0) // add (if not already in game)
            {
                word += "Recruit ";
                word += name;
                word += " as a Bot.";
