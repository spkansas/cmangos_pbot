    ...
    case SMSG_SPELL_GO:
        {
            ...
            else if (casterGuid == m_bot->GetObjectGuid())
            {
    ...
    case MSG_MOVE_TELEPORT_ACK:
        {
            ...
            if (m_bot->IsBeingTeleportedNear())
            {
                ...
                *p << (uint32) time(0); // time - not currently used
                m_bot->GetSession()->QueuePacket(std::move(std::make_unique<WorldPacket>(*p)));
                ...
                *p2 << mi;
                m_bot->GetSession()->QueuePacket(std::move(std::make_unique<WorldPacket>(*p2)));

                WorldPacket* const p3 = new WorldPacket(MSG_MOVE_FALL_LAND, 64);
                p3->appendPackGUID(m_bot->GetObjectGuid());
                *p3 << mi;
                m_bot->GetSession()->QueuePacket(std::move(std::make_unique<WorldPacket>(*p3)));
    ...
    case SMSG_NEW_WORLD:
        {
            ...
            if (m_bot->IsBeingTeleportedFar())
            {
                ...
                m_bot->GetSession()->QueuePacket(std::move(std::make_unique<WorldPacket>(*pt1)));
